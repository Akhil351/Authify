...
const Menubar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      toast.error(
        error?.response?.data?.error || "Server error during logout."
      );
    }
  };
  ...
  const sendVerificationOtp = async () => {
    try {
      ...
      if (status === "success") {
        ...
        toast.success(data || "OTP has been sent successfully.");
      } else {
        toast.error("Unable to send OTP!");
      }
    } catch (error) {
      toast.error(
        error?.response?.data?.error || "Server error during OTP sending."
      );
    }
  };
